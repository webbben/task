...
func (m *model) setTextContent() {
	if m.content == nil {
		return
	}
	if m.content.Notes == nil {
		m.textContent = "No notes found."
		return
	}

	var out string
	for date, note := range m.content.Notes {
		out = fmt.Sprintf("%s%s\n\n%s\n\n", out, date, note)
	}
	m.textContent = out
}
...
func (m model) headerView() string {
	title := titleStyle.Render(fmt.Sprintf("Notes: %s", m.content.Title))
